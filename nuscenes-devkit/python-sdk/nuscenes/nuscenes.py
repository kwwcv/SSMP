...
class NuScenes:
    ...
    def get_instance_box(self, sample_data_token: str, instance_token: str):
        """
        ...
        instance_ann_token = self.get_ann_of_instance(curr_sample_record, instance_token)
        if instance_ann_token == "":
            return None, None, None

        sample_ann_rec = self.get('sample_annotation', instance_ann_token)

        # Get the attribute of this annotation
        if len(sample_ann_rec['attribute_tokens']) != 0:
            attr = self.get('attribute', sample_ann_rec['attribute_tokens'][0])['name']
        else:
            attr = None
        ...
        if curr_sample_record['prev'] == "" or sd_record['is_key_frame']:
            # If no previous annotations available, or if sample_data is keyframe just return the current ones.
            box = self.get_box(instance_ann_token)

        else:
            prev_sample_record = self.get('sample', curr_sample_record['prev'])

            curr_ann_rec = self.get('sample_annotation', instance_ann_token)
            prev_ann_recs = [self.get('sample_annotation', token) for token in prev_sample_record['anns']]
    ...
    def get_ann_of_instance(self, sample_rec: Dict, instance_token: str) -> str:
        """
        ...
        for ann_token in sd_anns:
            tmp_ann_rec = self.get('sample_annotation', ann_token)
    ...
    def render_sample_data(self, sample_data_token: str, with_anns: bool = True,
                           ...
                           nsweeps: int = 1, out_path: str = None, use_future_sweeps: bool = False) -> None:
        self.explorer.render_sample_data(sample_data_token, with_anns, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
                                         out_path=out_path, use_future_sweeps=use_future_sweeps)
    ...
    def render_multisweep_multibox(self, sample_record: Dict, instance_token: str, view: np.ndarray = np.eye(4),
                                   margin: float = 40, render_multi_sweeps_lidar: bool = False, out_path: str = None):
        self.explorer.render_multisweep_multibox(sample_record, instance_token, view, margin,
                                                 render_multi_sweeps_lidar, out_path)

    def render_multisweep_instance_box_animation(self, nusc: 'NuScenes', sample_rec: Dict, instance_token: str,
                                                 freq: float = 10, view: np.ndarray = np.eye(4),
                                                 imsize: Tuple[float, float] = (640, 360), out_path: str = None):
        self.explorer.render_multisweep_instance_box_animation(nusc, sample_rec, instance_token,
                                                               freq, view, imsize, out_path)

    def render_multisweep_all_box_animation(self, nusc: 'NuScenes', sample_rec: Dict, freq: float = 10,
                                            view: np.ndarray = np.eye(4), imsize: Tuple[float, float] = (640, 360),
                                            out_path: str = None):
        self.explorer.render_multisweep_all_box_animation(nusc, sample_rec, freq, view, imsize, out_path)
...
class NuScenesExplorer:
    ...
    def render_sample_data(self,
        ...
        :param nsweeps: Number of sweeps for lidar and radar.
        :param use_future_sweeps: whether to include the future sweeps.
        :param out_path: Optional path to save the rendered figure to disk.
        """
        ...
        if sensor_modality == 'lidar':
            ...
            ref_chan = 'LIDAR_TOP'
            if use_future_sweeps:
                pc, times = LidarPointCloud.from_file_multisweep_bf(self.nusc, sample_rec, chan, ref_chan,
                                                                    nsweeps_back=nsweeps, nsweeps_forward=nsweeps,
                                                                    use_keyframe=True)
            else:
    ...
    def render_multisweep_multibox(self, sample_record: Dict,
                                   instance_token: str,
                                   view: np.ndarray = np.eye(4),
                                   margin: float = 20,
                                   render_multi_sweeps_lidar: bool = False,
                                   out_path: str = None) -> None:
        """
        Render multiple sweeps and bounding boxes. They are all mapped to the current sensor coordinate.
        :param sample_record: The sample record.
        :param instance_token: The instance id. This instance is to be rendered.
        :param view: LIDAR view point.
        :param margin: How many meters in each direction to include in LIDAR view.
        :param render_multi_sweeps_lidar: Whether to render the multisweep lidar data.
        :param out_path: Optional path to save the rendered figure to disk.
        """
        assert 'LIDAR_TOP' in sample_record['data'].keys(), 'No LIDAR_TOP in data, cant render'
        ...
        # Plot LIDAR view
        chan = 'LIDAR_TOP'
        lidar = sample_record['data'][chan]
        boxes, _, _, _ = LidarPointCloud.get_instance_boxes_multisweep(self.nusc, sample_record, instance_token, chan)

        if not render_multi_sweeps_lidar:
            data_path = self.nusc.get_sample_data_path(lidar)
            LidarPointCloud.from_file(data_path).render_height(axes, view=view)
        else:
            all_pc, _ = LidarPointCloud.from_file_multisweep_bf(self.nusc, sample_record, chan, chan)
            all_pc.render_height(axes, view=view)
        ...
        corners = view_points(boxes[0].corners(), view, False)[:2, :]
        axes.set_xlim([np.min(corners[0, :]) - margin, np.max(corners[0, :]) + margin])
        axes.set_ylim([np.min(corners[1, :]) - margin, np.max(corners[1, :]) + margin])
        axes.axis('off')
    ...
    def render_multisweep_instance_box_animation(self,
                                                 nusc: 'NuScenes',
                                                 sample_rec: Dict,
                                                 instance_token: str,
                                                 freq: float = 10,
                                                 ...
                                                 out_path: str = None) -> None:
        """
        Renders the multiple sweeps and instance box annotations for a sample data at certain time.
        :param nusc: A NuScenes instance.
        :param sample_rec: The current sample.
        :param instance_token: The instance to be rendered.
        :param freq: Display frequency (Hz).
        :param view: LIDAR view point.
        :param imsize: Size of image to render. The larger the slower this will run.
        :param out_path: Optional path to write a video file of the rendered frames.
        ...
        # Get the first (ie, current) and last sample records
        first_sample_rec = sample_rec
        last_sample_rec = self.nusc.get('sample', scene_rec['last_sample_token'])

        # Get the lidar data. For speed purpose, we only render the current sample data.
        lidar = sample_rec['data']['LIDAR_TOP']
        data_path = self.nusc.get_sample_data_path(lidar)
        instance_boxes, all_times, _, _ = LidarPointCloud.get_instance_boxes_multisweep(self.nusc,
                                                                                        sample_rec,
                                                                                        instance_token, 'LIDAR_TOP',
                                                                                        nsweeps_forward=sys.maxsize)
        ...
        # Recover the original time stamps of bounding boxes
        all_times = current_time - np.array(all_times) * 1e6
        ...
        while current_time < last_sample_rec['timestamp']:
            ...
            # find the first bounding box annotation that has time > current_time
            box_idx = 0
            while box_idx < len(all_times) and all_times[box_idx] < current_time:
                box_idx += 1
            ...
            # Show lidar data as well as the bounding box annotation
            LidarPointCloud.from_file(data_path).render_height(axes[0], view=view)

            if box_idx < len(instance_boxes) and instance_boxes[box_idx] is not None:
                c = np.array(self.get_color(instance_boxes[box_idx].name)) / 255.0
                instance_boxes[box_idx].render(axes[0], view=view, colors=(c, c, c))
                axes[0].axis('off')
                axes[0].set_aspect('equal')
    ...
    def render_multisweep_all_box_animation(self,
                                            nusc: 'NuScenes',
                                            sample_rec: Dict,
                                            freq: float = 10,
                                            view: np.ndarray = np.eye(4),
        ...
        """
        Renders the multiple sweeps and instance box annotations for a sample data at certain time.
        This is for testing and debugging purpose.
        :param nusc: A NuScenes instance.
        :param sample_rec: The current sample.
        :param freq: Display frequency (Hz).
        :param view: LIDAR view point.
        ...
        lidar = sample_rec['data']['LIDAR_TOP']
        data_path = self.nusc.get_sample_data_path(lidar)
        all_boxes_list, all_times = LidarPointCloud.get_all_boxes_multisweep(self.nusc,
                                                                             sample_rec,
                                                                             'LIDAR_TOP',
                                                                             nsweeps_forward=sys.maxsize)
        ...
        while current_time < last_sample_rec['timestamp']:
            ...
            # find the first batch of bounding box annotations that have time > current_time
            box_idx = 0
            while box_idx < len(all_times) and all_times[box_idx] < current_time:
                box_idx += 1
            ...
            # Show lidar data as well as the bounding box annotation
            LidarPointCloud.from_file(data_path).render_height(axes[0], view=view, x_lim=(-40, 40), y_lim=(-40, 40))

            if box_idx < len(all_boxes_list):
                tmp_boxes = all_boxes_list[box_idx]

                for tmp_box in tmp_boxes:
                    c = np.array(self.get_color(tmp_box.name)) / 255.0
                    tmp_box.render(axes[0], view=view, colors=(c, c, c))
                    axes[0].axis('off')
                    axes[0].set_aspect('equal')
