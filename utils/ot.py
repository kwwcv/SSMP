...
def Cost_Gaussian_function(data1, data2, theta_2=3, threshold_2=12):
    # data1: (B, 2, N), data2: (B, 2, N)
    distance2_matrix = torch.sum(data1 ** 2, 1, keepdims=True).transpose(2, 1) # B, N, 1
    distance2_matrix = distance2_matrix + torch.sum(data2 ** 2, 1, keepdims=True)  # B, N, N
...
def OT(C, epsilon=0.03, OT_iter=4):
    ...
    for _ in range(OT_iter):
        # Update b
        KTa = torch.matmul(K.transpose(2, 1), a)
        b = prob2 / (KTa + 1e-8)
    ...
    # Transportation map
    T = torch.multiply(torch.multiply(a, K), b.transpose(2, 1))
    return T