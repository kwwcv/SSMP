# modified based on 'data_utils.py' in MotionNet(https://www.merl.com/research/?research=license-request&sw=MotionNet)
import sys
...
def gen_2d_grid_gt(data_dict: dict, grid_size: np.array, extents: np.array = None,
    ...
    points_category = np.zeros(refer_pc.shape[0], dtype=np.int32)  # store the point categories

    pixel_instance_id = np.zeros(pixel_indices.shape[0], dtype=np.uint8)
    points_instance_id = np.zeros(refer_pc.shape[0], dtype=np.int32)
    ...
    past_part = list(range(0, num_past_sweeps, 3 + 1))[:5]
    ...
    for i in frame_considered:
        ...
        valid_pixels = np.zeros(unique_indices.shape[0], dtype=np.bool_)
...
def compute_ratio_cat_and_motion(dataset_root=None, frame_skip=3, voxel_size=(0.4, 0.4, 0.4), split='train',
    ...
    for idx in range(num_sample_seqs):
        ...
        pixel_cat_map = (pixel_cat_map * non_empty_map * filter_mask).astype(np.int32)
        ...
        motion_cat_map = (motion_cat_map * non_empty_map * filter_mask).astype(np.int32)
...
def voxelize_occupy_bevindices(pts, voxel_size, extents=None, filter=False):
    """
    Voxelize the input point cloud. We only record if a given voxel is occupied or not, which is just binary indicator.
    ...
    :param extents: Optional, specifies the full extents of the point cloud.
                    Used for creating same sized voxel grids. Shape (3, 2)
    :param return_indices: Whether to return the non-empty voxel indices.
    :param fills: Values as the same size as Point cloud as N x [x, y, z, i]
    """
    # Function Constants
    VOXEL_EMPTY = 0
    VOXEL_FILLED = 1
    ...
    # Get the voxel grid dimensions
    # num_divisions = ((max_voxel_coord[:2] - min_voxel_coord[:2]) + 1).astype(np.int32)

    # Bring the min voxel to the origin
    voxel_indices = (voxel_coords - min_voxel_coord[:2])#.astype(np.int32)
